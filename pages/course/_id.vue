      ...
      <div>
        <article class="c-v-pic-wrap" style="height: 357px">
          <section id="videoPlay" class="p-h-video-box">
            <img :height="357" :src="course.cover" :alt="course.title" class="dis c-v-pic" />
          </section>
        </article>
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt" v-if="Number(course.price)===0||buyCourse">
                <el-button round  type="warning"   @click="toPlay()">立即观看</el-button>
              <!-- <a @click="createOrder()" title="立即购买" class="comm-btn c-btn-3">立即购买</a> -->
            </section>

             <section class="c-attr-mt" v-else>
                <el-button round  type="warning"   @click="createOrder()">立即购买</el-button>
              <!-- <a @click="createOrder()" title="立即购买" class="comm-btn c-btn-3">立即购买</a> -->
            </section>
          </section>
        </aside>
            ...
            <div class="of">
              <section class="n-reply-wrap">
                ...
                <p class="of mt5 tar pl10 pr10">
                  ...
                  <el-button @click="addComment()" type="success">回复</el-button>
...
export default {
  ...
  methods:{
    ...
    //获取课程详情
    initCourseinfo(){
     course.getCourseDetail(this.courseId)
     .then(response=>{
       this.course=response.data.data.courseWebVo
       this.chapterVideoList=response.data.data.chapterVideoList
       this.buyCourse=response.data.data.buyCourse
     })
    },
    //获取评论详情
     initComment(){
       comment.getPageComment(this.page, this.limit,this.courseId).then(response => {
    ...
    gotoPage(page){
          comment.getPageComment(page, this.limit,this.courseId).then(response => {
              this.data = response.data.data
          })
      }
